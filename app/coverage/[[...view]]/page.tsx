...
import { CoverageViews, type ViewKey } from "@/components/coverage-views";
import { buildExploreData } from "@/lib/explore-data";

export const metadata: Metadata = {
  title: "Coverage · Neutral Risk",
  description:
    "Side-by-side coverage of the top Ethereum protocols by every major risk feed — verbatim, attributed, never scored.",
};

// One coverage browser, four views, each with its own URL: /coverage (list), /coverage/matrix,
// /coverage/map, /coverage/explore. The view is a real path segment (not a query param) so each is
// separately prerendered and content-addressable — the static / IPFS publishing story
// (docs/ROADMAP.md §4) wants distinct, cacheable pages rather than dynamic rendering.
export function generateStaticParams() {
  return [{ view: [] }, { view: ["matrix"] }, { view: ["map"] }, { view: ["explore"] }];
}

const VIEW_PATHS = new Set<ViewKey>(["matrix", "map", "explore"]);
...
export default async function CoveragePage({
  ...
  // Optional catch-all: undefined / [] is the default List view; a single known segment selects
  // that view; anything else (extra segments or an unknown slug) is a real 404, never a silent
  // fall-through to the default.
  if (view && view.length > 1) notFound();
  const seg = view?.[0];
  let initialView: ViewKey = "list";
  if (seg) {
    if (!VIEW_PATHS.has(seg as ViewKey)) notFound();
    initialView = seg as ViewKey;
  }
  ...
  return (
    <div>
      {/* Title + registry stats live in the global header strapline and top status bar
